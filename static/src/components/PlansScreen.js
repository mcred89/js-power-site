...
import { recipeDescription } from './StrongmanFields';
import { MaxCorrection as DefaultMaxCorrection, PlanSetup as DefaultPlanSetup, RoutineNameEditor as DefaultRoutineNameEditor } from './PlanControls';

export const isPlanComplete = routine => (
  (routine.kind === 'strongman' && routine.status === 'complete')
  || (routine.workouts.length > 0 && routine.workouts.every(workout => Boolean(workout.completedAt || workout.skippedAt)))
);
  ...
  <details className="completed-plan-details">
    ...
    <div className="completed-plan-contents">
      ...
      <div className="completed-plan-days">
        <h3>{routine.kind === 'strongman' ? 'Generated event days' : 'Generated workout days'}</h3>
        {routine.workouts.map(workout => <details className="completed-plan-day" key={workout.id}>
          ...
          <div className="exercise-list">
            {(workout.exercises || []).map(exercise => {
              ...
              if (routine.kind === 'strongman') return <div className="exercise-row" key={exercise.id}><div><strong>{shown.movement}</strong><span>{recipeDescription(exercise.generated.event)}</span></div></div>;
...
export const PlansScreen = ({ profile, routines, selectedId, templates, actions, RoutineNameEditor = DefaultRoutineNameEditor, MaxCorrection = DefaultMaxCorrection, PlanSetup = DefaultPlanSetup }) => (
  <section className="section-page">
    <div className="section-heading"><div><p className="eyebrow">{profile.name}</p><h1>Plans</h1></div><div className="button-row"><button className="primary-button small-primary" type="button" onClick={actions.newRoutine}>New routine</button>{actions.newStrongman && <button className="secondary-button" type="button" onClick={actions.newStrongman}>New strongman block</button>}</div></div>
    {!routines.length ? <p>No plans yet.</p> : routines.map(item => {
      const completed = isPlanComplete(item);
      const status = completed ? 'completed' : item.kind === 'strongman' ? item.status || 'saved' : 'active';
      const statusLabel = { active: 'Active', paused: 'Paused', saved: 'Saved', completed: 'Completed' }[status] || 'Saved';
      const skipped = item.workouts.filter(day => day.skippedAt).length;
      return <article className={`plan-card ${item.id === selectedId ? 'selected' : ''} ${status}`} key={item.id}>
        <button className="plan-select" type="button" onClick={() => actions.select(item)} aria-label={`View ${item.name}`}><span><strong>{item.name}</strong><small>{item.workouts.filter(day => day.completedAt).length} of {item.workouts.length} complete{item.kind === 'strongman' && skipped > 0 ? ` · ${skipped} skipped` : ''}</small></span><span className={`plan-status ${status}`}>{statusLabel}</span></button>
        ...
        {item.id === selectedId && !completed && item.kind !== 'strongman' && <MaxCorrection routine={item} onCorrect={maxes => actions.correct(item, maxes)} />}
      </article>;
    })}
